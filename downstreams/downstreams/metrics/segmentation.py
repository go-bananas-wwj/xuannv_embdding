...
def compute_segmentation_metrics_at_threshold(
    pred_logits: torch.Tensor | np.ndarray,
    target: torch.Tensor | np.ndarray,
    threshold: float,
    ignore_index: int = -1,
) -> dict[str, Any]:
    """在指定阈值下计算分割指标（不含 PR 曲线）。"""
    pred_logits = _ensure_numpy(pred_logits)
    target = _ensure_numpy(target)
    valid = target != ignore_index
    probs = 1.0 / (1.0 + np.exp(-pred_logits))
    preds = (probs > threshold).astype(np.uint8)
    y_true = (target == 1) & valid
    y_pred = (preds == 1) & valid
...
def find_best_threshold(
    ...
    for thr in thresholds:
        m = compute_segmentation_metrics_at_threshold(pred_logits, target, thr, ignore_index)
