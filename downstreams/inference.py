...
def build_inference_loader(cfg: Config, region: str, split: str = "all") -> DataLoader:
    ...
    if cfg.data.statistics_dir is not None:
    ...
    dataset = MonthlyEmbeddingDataset(
        ...
        num_months=cfg.model.num_months,
        ref_year=cfg.data.ref_year,
        ref_month=cfg.data.ref_month,
        region=region,
    )
        ...
        for batch in loader:
            ...
            for b, patch_id in enumerate(patch_ids):
                ...
                for m in range(emb_map.shape[1]):
                    ...
                    torch.save(emb_map[b, m].detach().contiguous().clone(), patch_dir / f"{month_int}_embedding_map.pt")
                    torch.save(scene_emb[b, m].detach().contiguous().clone(), patch_dir / f"{month_int}_scene_embedding.pt")
