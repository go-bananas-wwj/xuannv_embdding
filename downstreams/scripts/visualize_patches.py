...
def _find_latest(path: Path) -> Path | None:
    candidates = sorted(path.parent.glob(path.name + "_*"), key=lambda p: p.stat().st_mtime)
    if candidates:
        return candidates[-1]
...
def _load_s2_rgb(patch_id: str, root: Path) -> np.ndarray | None:
    ...
    rgb = np.stack([im[3], im[2], im[1]], axis=-1)  # NIR/R/G -> use R/G/B bands 3,2,1
    return _stretch(rgb)
...
def _compute_embedding_pca(emb: np.ndarray, n_components: int = 3) -> np.ndarray:
    """emb: (C, H, W) -> (H, W, 3)"""
    c, h, w = emb.shape
...
def visualize_patch(
    ...
    pca_rgb = _compute_embedding_pca(emb)
...
def main() -> None:
    ...
    p.add_argument("--data-root", type=Path, required=True, help="processed/haidian")
