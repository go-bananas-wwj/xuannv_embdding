...
def apply_augment(
    ...
    """对 tensor (C, H, W) 和 mask (H, W) 做同步随机增强。"""
    # 同步水平翻转
    ...
    if crop_size is not None:
        tensor, mask = random_crop(tensor, mask, crop_size)
    return tensor, mask


def random_crop(
    tensor: torch.Tensor, mask: torch.Tensor, crop_size: int
) -> tuple[torch.Tensor, torch.Tensor]:
    """对 tensor (C, H, W) 和 mask (H, W) 做同步随机裁剪。
    ...
    y2 = y1 + crop_size
    x2 = x1 + crop_size
    return tensor[:, y1:y2, x1:x2], mask[y1:y2, x1:x2]
...
class MultiTaskEmbeddingDataset(Dataset):
    ...
    def _apply_augment(
        ...
        return apply_augment(emb, mask, self._crop_size)

    @staticmethod
    def _random_crop(
        emb: torch.Tensor, mask: torch.Tensor, crop_size: int
    ) -> tuple[torch.Tensor, torch.Tensor]:
        return random_crop(emb, mask, crop_size)
