...
def _months() -> list[str]:
    return [f"{year:04d}-{month:02d}" for year, start, end in ((2025, 4, 12), (2026, 1, 4)) for month in range(start, end + 1)]


def _interval(month: str) -> str:
    year, month_number = (int(value) for value in month.split("-"))
    last_day = monthrange(year, month_number)[1]
    return f"{date(year, month_number, 1).isoformat()}T00:00:00Z/{date(year, month_number, last_day).isoformat()}T23:59:59Z"
...
def _compact_feature(feature: dict[str, Any], asset_names: list[str]) -> dict[str, Any]:
    ...
    return {
        ...
        "properties": {
            ...
            for key in ("datetime", "start_datetime", "end_datetime", "eo:cloud_cover", "proj:code", "proj:epsg", "sat:orbit_state", "s1:instrument_configuration_ID")
            if key in feature.get("properties", {})
        },
        "assets": {
            name: {key: assets[name].get(key) for key in ("href", "type", "roles", "title", "raster:bands") if key in assets[name]}
            for name in asset_names if name in assets
        },
    }


def _next_request(payload: dict[str, Any], page: dict[str, Any]) -> tuple[str, str, dict[str, Any] | None] | None:
...
def cache_month(session: requests.Session, source: str, month: str, bbox: list[float], output_root: Path, limit: int) -> dict[str, Any]:
    ...
    payload = {
        ...
        "fields": {"include": ["id", "collection", "bbox", "geometry", "properties.datetime", "properties.start_datetime", "properties.end_datetime", "properties.eo:cloud_cover", "properties.proj:code", "properties.proj:epsg", "properties.sat:orbit_state", "properties.s1:instrument_configuration_ID", "assets"]},
    }
    ...
    request: tuple[str, str, dict[str, Any] | None] | None = ("POST", f"{CATALOG_URL}/search", payload)
    count = 0
    pages = 0
    ...
    status = {
        "schema_version": "china_v1_stac_cache_v1", "complete": True, "source": source,
        "collection": config["collection"], "month": month, "bbox": bbox,
        "query_sha256": query_hash, "item_count": count, "page_count": pages,
        "elapsed_seconds": round(time.monotonic() - started, 3), "catalog_url": CATALOG_URL,
    }
