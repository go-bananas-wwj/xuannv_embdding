...
def _interval(month: str) -> str:
    ...
    return f"{date(year, number, 1).isoformat()}T00:00:00Z/{date(year, number, monthrange(year, number)[1]).isoformat()}T23:59:59Z"
...
def _compact_feature(feature: dict[str, Any], source: str) -> dict[str, Any] | None:
    ...
    for target_name, cdse_name in config["asset_map"].items():
        ...
        if not asset or not str(asset.get("href", "")).startswith("s3://eodata/"):
            return None
        assets[target_name] = {key: asset[key] for key in ("href", "type", "roles", "title") if key in asset}
    properties = feature.get("properties", {})
    return {
        "id": feature["id"], "collection": feature.get("collection"), "bbox": feature.get("bbox"),
        "geometry": feature.get("geometry"),
        "properties": {key: properties.get(key) for key in (
            "datetime", "start_datetime", "end_datetime", "eo:cloud_cover", "proj:code",
            "proj:epsg", "sat:orbit_state", "s1:instrument_configuration_ID",
...
def _request_with_retry(session: requests.Session, method: str, url: str, body: dict[str, Any] | None, attempts: int = 6) -> requests.Response:
...
def cache_month(session: requests.Session, source: str, month: str, bbox: list[float], output_root: Path, limit: int) -> dict[str, Any]:
    ...
    status = {
        "schema_version": "china_v1_cdse_stac_cache_v1", "complete": True, "source": source,
        "collection": config["collection"], "month": month, "bbox": bbox, "query_sha256": query_hash,
        "item_count": count, "skipped_incomplete_assets": skipped, "page_count": pages,
