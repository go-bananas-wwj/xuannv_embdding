...
def _spatial_order(points: list[dict[str, object]]) -> list[dict[str, object]]:
    return sorted(points, key=lambda point: (str(point.get("grid_id", "")), int(point.get("grid_row", 0)), int(point.get("grid_col", 0)), str(point["patch_id"])))
...
def main() -> None:
    ...
    parser.add_argument("--order", choices=("random", "spatial"), default="random")
    ...
    if args.order == "spatial":
        selected = _spatial_order(selected)
    ...
    manifest = {
        ...
        "shard_size": args.shard_size, "seed": args.seed, "prefix": args.prefix, "order": args.order,
