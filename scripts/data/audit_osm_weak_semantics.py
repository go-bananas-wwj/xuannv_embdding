...
CATEGORY_RULES: dict[str, tuple[tuple[str, set[str] | None], ...]] = {
    ...
    "road": (("highway", {"motorway", "trunk", "primary", "secondary", "tertiary", "unclassified", "residential", "service", "living_street", "pedestrian", "track", "path", "footway", "cycleway", "steps", "bridleway", "construction"}),),
    "rail": (("railway", {"rail", "light_rail", "subway", "tram", "narrow_gauge", "monorail"}),),
    "water": (("natural", {"water", "wetland"}), ("water", None), ("waterway", {"riverbank", "canal", "river", "stream", "drain"}), ("landuse", {"reservoir", "basin"})),
    "green": (("leisure", {"park", "garden", "nature_reserve", "common"}), ("landuse", {"forest", "grass", "meadow", "recreation_ground", "village_green"}), ("natural", {"wood", "grassland", "scrub", "heath"})),
    "agriculture": (("landuse", {"farmland", "farmyard", "orchard", "vineyard", "plant_nursery", "greenhouse_horticulture"}),),
    "education": (("amenity", {"school", "university", "college", "kindergarten"}),),
    "sports": (("leisure", {"pitch", "stadium", "sports_centre", "playground", "track"}), ("amenity", {"sports_centre"})),
    "industrial": (("landuse", {"industrial", "commercial", "retail"}),),
    "construction": (("landuse", {"construction", "quarry", "landfill"}), ("highway", {"construction"})),
    "airport_port": (("aeroway", {"aerodrome", "runway", "taxiway", "apron", "terminal"}), ("man_made", {"pier", "breakwater"}), ("harbour", None)),
}
TRACKED_KEYS = ("building", "highway", "railway", "natural", "water", "waterway", "landuse", "leisure", "amenity", "aeroway", "man_made", "harbour")
...
def _scan(path: Path) -> dict[str, object]:
    try:
        ...
        raise RuntimeError("Install the optional 'osmium' package before running this audit.") from exc
...
def main() -> None:
    ...
    report = {
        ...
        "category_rules": {category: [[key, sorted(values) if values else None] for key, values in rules] for category, rules in CATEGORY_RULES.items()},
