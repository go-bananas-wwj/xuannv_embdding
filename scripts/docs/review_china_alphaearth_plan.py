...
def main() -> None:
    ...
    with zipfile.ZipFile(SOURCE) as source_zip:
        ...
        comments = [
            ...
            (
                "东北训练范围：80.8 万 km² × 30% = 24.2 万 km²；须说明 30% 的分层抽样规则、覆盖生态区和每月有效观测阈值 | 名义训练窗口：107 天",
                "“30%”不能只是面积比例。数据审查阶段建议使用二维 10×10 系统抽样：真实网格横向、纵向各每 10 个 patch 取 1 个，即 1%，并在各分层内随机起始相位；不可简化为每 100 个编号抽一个。附录还需列出省份/生态区/土地覆盖/城乡梯度/云量带，并固定每月最小有效像素比例。否则训练集可能集中在易获取、低云区域，无法代表中国尺度泛化。",
            ),
            (
                "建议：优先建设 10 m 东北试点底座，先用固定优化步数的 1,000-step 吞吐基准和数据就绪率复算工期，再作为团队技术验证与首版成果。",
            ...
            (
                "2 m 目标不应与全国 10 m 主预训练并列承诺：其主要瓶颈是高分数据授权与覆盖、存储和 I/O，而非单纯卡数；建议改为完成 10 m 主底座后的重点城市/行业区域增强路线。",
                "请按当前实际导出格式，而不是理论FP16格式估算：P10C 的 64维、128×128 embedding map 目前以 FP32 保存，海淀320 patch、6个月实测7.51GiB。线性外推30%全国、6个月，10 m约44 TB；5 m约177 TB；2 m约1.11 PB，尚未包括输入、临时缓存、版本和副本。验证FP16导出后才可将这些值约减半。因此 2 m 必须单列数据、存储和交付方案。",
            ),
            (
                "如需 107 天完成（线性外推，未计多节点效率损失）",
            ...
            (
                "中国版 AlphaEarth：10 m 国家级地球观测嵌入的分阶段建设方案",
                "建议在首页增加一句定位：本项目借鉴 AlphaEarth 的“多源观测→统一 embedding→少标签下游制图”范式，但不是对官方模型的复刻或性能等价声明。AEF公开训练使用512 TPU v4、56小时、10万步，发布年度10m、64维、8-bit量化embedding；其训练原始数据与发布桶的总存储均未公开。本文可突出中国数据源和月度更新能力，但需单独核算月度数据质量和FP32/FP16/8-bit发布格式的存储成本。",
            ),
            (
                "方案一5m可行性",
