...
def build() -> Presentation:
    ...
    text(s, "同一套地理嵌入，进入中国城市治理场景后，必须适配本土数据、标签口径和业务目标。", 1.02, 1.14, 11.20, 0.28, 13, C.body, False, PP_ALIGN.CENTER)
    ...
    rect(s, 0.86, 5.36, 5.20, 0.46, C.pale_blue, C.blue)
    text(s, "施工工地、建筑变化、垃圾堆放等任务，依赖本地标签体系和业务定义。", 1.04, 5.50, 4.84, 0.14, 9, C.body, True, PP_ALIGN.CENTER)
    line(s, 6.42, 1.54, 6.42, 6.16, C.line, 0.9)
    text(s, "三点判断", 6.86, 1.62, 5.10, 0.20, 14, C.blue, True, PP_ALIGN.CENTER)
    rect(s, 6.92, 2.12, 4.86, 0.66, C.pale_blue, C.blue)
    text(s, "1 方向成立", 7.16, 2.26, 1.24, 0.14, 10, C.blue, True)
    text(s, "AlphaEarth 验证地理嵌入的基础设施价值。", 8.54, 2.26, 3.04, 0.14, 9, C.ink, True)
    rect(s, 6.92, 3.30, 4.86, 0.66, C.mint, C.green)
    text(s, "2 本土差异", 7.16, 3.44, 1.24, 0.14, 10, C.green, True)
    text(s, "多时序城市变化任务，玄女已有可量化优势。", 8.54, 3.44, 3.04, 0.14, 9, C.ink, True)
    rect(s, 6.92, 4.48, 4.86, 0.66, C.pale_amber, C.amber)
    text(s, "3 数据壁垒", 7.16, 4.62, 1.24, 0.14, 10, C.amber, True)
    text(s, "单时序短板通过本土数据闭环持续补齐。", 8.54, 4.62, 3.04, 0.14, 9, C.ink, True)
    text(s, "本土数据 + 本土标签 + 本土业务闭环，形成中国城市治理场景下的地理智能底座。", 0.86, 6.72, 11.60, 0.22, 14, C.blue, True, PP_ALIGN.CENTER)
