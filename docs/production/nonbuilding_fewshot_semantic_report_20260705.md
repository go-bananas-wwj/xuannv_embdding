...
本报告展示玄女海淀 V1 embedding 在海淀区非建筑语义类别上的少样本制图能力。评估重点从建筑物扩展到公园、操场/球场、教育区域、体育设施和草地/绿地等城市功能语义，体现模型在复杂城市地物中的表达能力。
...
`park / 公园` 与 `pitch / 操场球场` 是当前最适合展示的非建筑语义类别。在 5 个和 10 个正样本 patch 的极少样本设置下，玄女相对 AEF 有更明显优势。
...
图 1. 公园类别的海淀全域 320 patch 制图。每一行对应 5、10、50 个正样本 patch 训练后的结果；三列分别为 OSM 标注参考、玄女预测、AEF 预测。

![公园patch叠加](assets/nonbuilding_fewshot_semantic_20260705/park_patch_examples_50shot.png)

图 2. 公园类别典型 patch 展示。第一列为高分辨率光学影像，其余列分别叠加 OSM 标注、玄女预测和 AEF 预测。
...
图 5. 操场 ROI 相似区域检索示例。左侧红框为用户圈选的操场区域，右侧为玄女和 AEF 分别返回的 Top-8 相似候选 patch。该能力可用于从一个已知样例快速发现全区相似地物候选，再交由人工复核。
