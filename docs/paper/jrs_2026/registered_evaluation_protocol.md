...
- Use a 10,000-replicate paired hierarchical block bootstrap: resample the five folds with replacement, then resample 2 x 2 geographic patch clusters within each selected fold, preserving all paired model predictions and averaging the three downstream seeds inside each replicate.
...
|---|---|---|
| G1 | Evaluation CLI accepts and records the registered spatial split path/hash | FAIL: legacy scripts read task-local random splits |
| G2 | Exact shot manifests are generated and insufficient budgets raise an error | FAIL: legacy sampler silently truncates |
| G3 | Exact 40/80/150 subset registry exists and is hash-checked | PASS: generated by `scripts/experiments/build_paper_subset_registry.py` |
