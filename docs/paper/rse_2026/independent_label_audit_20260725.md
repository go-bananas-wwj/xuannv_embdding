...
labels are available over all 320 patches, but they overlap semantically with the OSM-derived
merged land-cover supervision used by the full recipe and have a 2023 versus 2025--2026 time
mismatch.
...
- The full registered recipe nevertheless includes an OSM-derived merged land-cover categorical
  target and fine OSM semantic probes. A downstream task sharing those ontologies is not
  task-ontology-independent.
...
| --- | --- | --- | --- |
| ESRI 2023 LULC | 320/320 patches, 10 m | Annual land-cover diagnostics: water, trees, crops, bare, rangeland, built area | Semantic overlap with the full recipe's OSM-derived merged land-cover target; 2023 temporal mismatch; built area is not a building footprint |
...
2. Evaluate `no_osm_150` against ESRI water and land-cover masks as a reconstruction-only
   diagnostic. It did not train on OSM-derived semantic targets, but ESRI's annual time
   mismatch and land-cover semantics remain limitations.
3. Compare the full recipe, merged-OSM-land-cover-only recipe, and no-OSM recipe on ESRI labels only as an
